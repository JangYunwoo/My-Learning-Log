...
def execute_tool(tool_call):
    ...
    # 딕셔너리에서 실제 함수를 꺼낸다. 등록되지 않았다면 None이다.
    ...
    # 꺼낸 함수에 모델이 전달한 인자를 넣어 실행한다.
    return tool_function(**arguments)

    # 기존 if문에서 dict형태로 호출하기 위해 삭제
    '''
...
async def request_wiki_update_step(messages: list):
    response = await client.chat(
        model="qwen3:4b",
        messages=messages,
        tools=[
...
# 기존 위키를 읽고 수정한 뒤 최종 답변할 때까지 반복한다.
async def run_wiki_update_agent(user_request: str) -> str:
    messages = [
        # 이전 수정 프롬프트
        # {"role": "system", "content": WIKI_UPDATE_SYSTEM_PROMPT},

        # 제목과 참고 문서의 중복을 방지한 수정 프롬프트
        {"role": "system", "content": WIKI_UPDATE_SYSTEM_PROMPT2},
        {"role": "user", "content": user_request},
    ]

    # 읽기, 원문 확인, 수정, 최종 답변을 위해 최대 8단계 실행한다.
    for step in range(8):
        print(f"wiki Update Agent 단계 {step + 1}")

        # 지금까지의 기록을 보고 다음 행동을 요청한다.
        agent_message = await request_wiki_update_step(messages)
        messages.append(agent_message)

        # 도구 요청이 없으면 최종 답변으로 보고 종료한다.
        if not agent_message.tool_calls:
            return agent_message.content

        # 모뎅리 요청한 도구들을 하나씩 실행한다.
        for tool_call in agent_message.tool_calls:
            tool_result = execute_tool(tool_call)
    ...
    raise RuntimeError("Wiki Update Agent가 최대 실행 횟수를 초과했습니다.")